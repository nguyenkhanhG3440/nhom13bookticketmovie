...
router.post("/create-momo-payment", async (req, res) => {
    try {
        ...
        // Chuyển danhSachGhe thành chuỗi JSON nếu chưa phải
        ...
        const requestBody = {
            ...
            ipnUrl,
            requestType,
            extraData, // Gửi extraData chứa danhSachGhe
            signature,
            lang: "vi"
        };
...
router.get('/payment-success', async (req, res) => {
    try {
        ...
        // Parse danhSachGhe từ chuỗi JSON thành mảng
        let danhSachGhe;
        try {
        ...
        if (resultCode === '0') {
            let totalPrice = Number(amount); // Chuyển amount thành số nếu cần

            // Cập nhật trạng thái ghế trong ChiTietPhongChieu
            const updateResult = await ChiTietPhongChieu.updateMany(
                {
                    phong: phongId, // Tìm các document có phongId khớp
                    ghe: { $in: danhSachGhe } // Tìm các ghế trong danhSachGhe
                },
                { $set: { trangThai: "Đã đặt" } } // Cập nhật trạng thái
            );
...
router.post("/create-momo-payment", paymentController.createMomoPayment);
